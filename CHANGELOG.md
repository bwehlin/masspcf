...
* **Slicing a point cloud shares its coordinates** — tensors and point clouds now follow one indexing contract, so `X[1:4]` and `X[1:4, 0:2]` mean the same thing on either. Selecting whole points from a cloud (a slice, an integer array, or a boolean mask) returns a `PointCloud` *view* that shares the original coordinates rather than copying them — the same representation `subsample_relative` produces, so a sliced cloud feeds straight into persistent homology without being materialized. Indexing that drops the point axis (`pc[0]`) or reaches the coordinate axis (`pc[:, 0]`) still materializes to a `FloatTensor`, so the plotting idiom is unchanged.
* **Indexed point clouds** — subsamples are returned as *indexed views* that share the reference cloud's coordinates and store only the chosen row indices, rather than re-storing every (possibly high-dimensional) point. A single `PointCloud` element exposes `.is_indexed`, `.indices`, `.shape`, and `.materialize()`; its coordinates are only copied when it is converted to NumPy. A `PointCloud` can be stored into a `PointCloudTensor` cell (`T[i] = cloud`) and stays indexed, so moving a subsample between tensors copies only its index array. Persistent homology consumes indexed subsamples directly, so the full sampling → persistence pipeline never materializes all subsamples at once. Point clouds are now first-class (rank-2 `(n_points, dim)`) objects backed by `sb::PointCloud`.
